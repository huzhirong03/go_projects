...
- **CSV 分隔符自动嗅探**：界面上 "CSV → 分隔符 → 自动推断" 现在真的会自动识别 4 种分隔符
  （`,` / `;` / `\t` / `|`），而不是像 v1.4.x 一样默认按逗号处理。
  - 实现位置：`internal/excelio/csv_reader.go` 的新 `sniffDelimiter` 函数，
    从文件前 8 KB 用真实 csv.Reader 切前 50 行评分，选"字段数 ≥ 2 且行间一致性 ≥ 90%"
    的候选；同分时按 `,` > `;` > `\t` > `|` 优先级。这是 Python `csv.Sniffer` /
    R `read_csv` / Excel 导入向导都在用的标准做法。
  - 嗅探发生在编码解码层之后，UTF-8 / UTF-8 BOM / GBK / GB18030 / UTF-16 等所有支持
    编码下都正确工作。
  - 解决用户反馈：v1.4.1 之前拿到 Tab 或分号 CSV 后留默认"自动推断"会导致整行内容
    挤进 A 列。现在零配置即可正确读取。
- **新 API `Reader.Delimiter() rune`**：日志和 UI 反馈用，能拿到嗅探出的实际分隔符。
- **Fixture 07 自检测试转为完整自动嗅探路径**：`TestGen07_CSVFixtures` 不再
  对分号/Tab 文件显式传 delimiter，5 个变体全靠自动嗅探，作为防回归基线。

### 行为变化 (Changed)

- `pickDelimiter` 语义微调：原本 `""` / `"auto"` / `","` / `"comma"` 都返回 0
  （走默认逗号）；现在 `","` / `"comma"` 返回 `','`（用户显式选了逗号、不再嗅探），
  仅 `""` / `"auto"` 触发嗅探。**显式指定的所有路径与 v1.4.x 完全等价**，零回归。
